...
def train (model, datasets, dataloaders, modelpath,
    ...
    args.model_subpath = 'cifar10' if args.num_classes == 10 else 'cifar100'
    ...
    for epoch in range(args.t1):
        running_loss = 0.0
        args.t1_iter_per_epoch = math.ceil(len(labeled_dataset) / args.train_batch)
        model.train()
        for i in range(args.t1_iter_per_epoch):
            try:
            ...
            running_loss += total_loss.item() * x_l.size(0)
        training_loss = running_loss/(args.t1_iter_per_epoch * args.train_batch)
        losses['training'].append(training_loss)
        print('Epoch: {} : Train Loss : {:.5f} '.format(
            epoch, training_loss))
        
        if validation:
            best_model = validate_training(epoch, model, criterion, optimizer, modelpath, losses, validation_dataset, validation_loader, best_model, args)
        
        # update learning rate
        scheduler.step()

        if test:
            test_training(epoch, model, criterion, losses, test_dataset, test_loader)
      

    # STAGE TWO -> args.t1 <= epoch <= args.epoch
    # alpha gets calculated for weighting the pseudolabeled data
    # we train over labeled and pseudolabeled data
    # we train 1 iteration over labeled set every 50 iterations on unlabeled (sizes are very different)
    
    for epoch in range(args.t1, args.epoch):
        running_loss = 0.0
        training_samples = 0
        model.train()
        for i in range(args.iter_per_epoch):
            # 4 calculations of labeled data every epoch
            if (args.iter_per_epoch % (args.iter_per_epoch/4) == 0):
            ...
            # calculate loss for pseudo-labelled data  
            model.train()
            ...
            # if there are no images computed in this epoch we dont do the backward pass
            # this is possible in the iterations when we dont train labeled data and
            # when no unlabelled data passes the threshold
            # no labelled nor pseudolabeled data
            if (l_loss != 0.0 or pl_loss != 0.0):
                # sum up labelled and unlabelled loss
                ...
                running_loss += total_loss.item() * (n_x_pl+n_x_l)
                training_samples += (n_x_pl+n_x_l)
            else:
                optimizer.zero_grad()
                
        training_loss = running_loss / training_samples
        losses['training'].append(training_loss)
        print('Epoch: {} : Train Loss : {:.5f} '.format(
            epoch, training_loss))
    ...
    last_model = {
        ...
        'model_state_dict': copy.deepcopy(model.state_dict()),
        'optimizer_state_dict': copy.deepcopy(optimizer.state_dict()),
        'training_losses':  copy.deepcopy(losses['training']),
        'validation_losses': copy.deepcopy(losses['validation']),
        'test_losses': copy.deepcopy(losses['test']),
        'model_depth' : args.model_depth,
        'num_classes' : args.num_classes,
        'model_width' : args.model_width,
        'drop_rate' : args.drop_rate
    }
    torch.save(last_model, pjoin(modelpath, 'last_model_{}.pt'.format(args.model_subpath)))
    if validation:
...
def validate_training(epoch, model, criterion, optimizer, modelpath, losses, validation_dataset, validation_loader, best_model, args):
...
# TODO implement early stopping?
    ...
    if best_model['epoch'] == -1 or validation_loss < best_model['validation_losses'][-1]:
        best_model = {
            'epoch': epoch,
            'model_state_dict': copy.deepcopy(model.state_dict()),
            'optimizer_state_dict': copy.deepcopy(optimizer.state_dict()),
        ...
        torch.save(best_model, pjoin(modelpath, 'best_model_{}.pt'.format(args.model_subpath)))
        print('Best model updated with validation loss : {:.5f} '.format(validation_loss))
    return best_model

  

def test_training(epoch, model, criterion, losses, test_dataset, test_loader):
